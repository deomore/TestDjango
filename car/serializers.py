...
class GameSerializer(serializers.ModelSerializer):
    ...
    categories_details = CategorySerializer(
        source="categories", many=True, read_only=True
    )
    comments_count = serializers.IntegerField(source="comments.count", read_only=True)
    # Поля ДЛЯ ЗАПИСИ СО СТОРОНЫ ФРОНТА
    studio = serializers.PrimaryKeyRelatedField(
        queryset=Studio.objects.all(), write_only=True
    )
...
class CommentsSerializer(serializers.ModelSerializer):
    ...
    about_game = serializers.SlugRelatedField(
        read_only=True, slug_field="name", source="game"
    )
